<input type="hidden" name="boundaries" value="{{$governorate->boundaries ?? '{}'}}" class="boundaries-input">
<div class="col-12">
    <div class="col-12">
...
@section('footer')
    ...
    <script>
        $(".country-select").on("change", function () {
            ...
            $.ajax({
                url: route('admin.countries.country.governorates', {country}),
                success: function (data) {
                    $(".governorate-select").html(data).removeAttr("disabled");
                }
            })
        });
        $(".governorate-select").on("change", function () {
            let lat = $(this).find("option:selected").data("lat");
            let lng = $(this).find("option:selected").data("lng");
            let zone = $(this).find("option:selected").data("boundaries");
            restMap();
