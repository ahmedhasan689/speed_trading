                ...
                @foreach($cart as $single_item)
                    <div class="d-flex justify-content-between flex-column flex-md-row gap-3">
                        <div class="d-flex gap-2">
                            <button type="button" class="btn-close m-auto deleteItem" aria-label="Close" data-id="{{ $single_item->item->id }}"></button>
                        ...
                        <div class="d-flex gap-4 mt-auto" style="height: fit-content;">
                            <div class="d-flex align-items-center gap-3">
                                <button class="btn btn-light p-0 rounded-3 plus"  data-id="{{ $single_item->item->id }}">
                                    <img src="{{ asset('assets/icon/chevron_right.svg') }}" width="30">
                                </button>
                                <span class="fw-bold quantity-{{ $single_item->item->id }}">
                                                    {{ $single_item->quantity }}
                                                </span>
                                <button class="btn btn-light p-0 rounded-3 loss loss-{{ $single_item->item->id }}" data-id="{{ $single_item->item->id }}" @if($single_item->quantity == 1) disabled @endif >
                                    <img src="{{ asset('assets/icon/chevro_left.svg') }}" width="30">
                                </button>
                            </div>
                            <span class="badge p-2 rounded-3 text-main-color price-{{ $single_item->item->id }}">{{ $single_item->quantity * $single_item->item->price }}L.E</span>
                        </div>
                    </div>
